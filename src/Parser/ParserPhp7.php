...
class ParserPhp7 {
   ...
   * @param array $attributesComments
  ...
  public function parseFunction(array $tokens, int &$pos, ?string $class, string $terminatedNamespace, array $attributesComments, bool $allowAnonymous = FALSE): \Iterator {
    ...
    if ($id === T_STRING) {
      $shortName = $tokens[$i][1];
      if ($attributesComments) {
        $symbol = ($class === NULL)
          ? SymbolHandle::fromFunction($terminatedNamespace . $shortName)
          : SymbolHandle::fromMethod($class, $shortName);
    ...
    if ($shortName !== NULL) {
      ...
      foreach ($this->parseParams($tokens, $i) as $paramDollarName => $paramAttributes) {
        $symbol = ($class === NULL)
          ? SymbolHandle::fromFunctionParameter(
            $terminatedNamespace . $shortName,
            substr($paramDollarName, 1))
          : SymbolHandle::fromMethodParameter(
            $class,
            $shortName,
            substr($paramDollarName, 1));
        yield $symbol => RawSymbolInfo::forInnerSymbol($attributesComments);
      ...
      for (++$i;; ++$i) {
        ...
        if (is_string($token)) {
          switch ($token) {
            ...
            case '}':
            case ']':
              throw SyntaxException::fromTokenPos($tokens, $i, "Unexpected token in function return type.");
    ...
    throw SyntaxException::fromTokenPos($tokens, $i, 'Unexpected token after function parameters.');
  }
