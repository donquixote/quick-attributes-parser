<?php /** @noinspection PhpIllegalPsrClassPathInspection */

declare(strict_types=1);

namespace Donquixote\QuickAttributes\Util;

class VersionDependentTokens {

  const T_ATTRIBUTE = 10383;

  const T_NAME_QUALIFIED = 10314;

  const T_NAME_FULLY_QUALIFIED = 10312;

  const T_AMPERSAND_FOLLOWED_BY_VAR_OR_VARARG = 10403;

  const T_AMPERSAND_NOT_FOLLOWED_BY_VAR_OR_VARARG = 10404;

  const T_STRING_7 = 70262;

  const T_STRING_8 = 80262;

  const T_NS_SEPARATOR_7 = 70398;

  const T_NS_SEPARATOR_8 = 80398;

}
